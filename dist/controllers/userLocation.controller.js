import { LocationType, AttendanceType, AttendanceSession, } from "../../generated/presence/index.js";
...
export const saveFieldTrips = async (req, res) => {
    try {
        ...
        const result = await localPrisma.$transaction(async (tx) => {
            ...
            if (fieldTripDates.length > 0) {
                const fieldTrips = await Promise.all(fieldTripDates.map(async (trip) => {
...
export const getFieldTrips = async (req, res) => {
    try {
...
export const getUserFieldTrips = async (req, res) => {
    try {
        const { employeeNumber } = req.params;
        if (!employeeNumber) {
            return res.status(400).json({
...
export const getUserFieldTripsByUsername = async (req, res) => {
    try {
        ...
        const isOnFieldTrip = fieldTrips.some((trip) => {
...
export const getUserFieldTripsByEmployeeNumber = async (req, res) => {
    try {
        const { employeeNumber } = req.params;
        if (!employeeNumber) {
            return res.status(400).json({
...
export const checkAndDeactivateExpiredFieldTrips = async (req, res) => {
    try {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const expiredFieldTrips = await localPrisma.fieldTrip.findMany({
            where: {
                isActive: true,
                endDate: {
                    lt: today,
...
export const processFieldTripAttendance = async (req, res) => {
    try {
            ...
            if (!existingAttendance) {
                await localPrisma.attendance.create({
                    data: {
                        employeeNumber: trip.employeeNumber,
                        date: today,
                        checkinTime: new Date(today.getFullYear(), today.getMonth(), today.getDate(), 9, 30),
...
export const deleteFieldTrip = async (req, res) => {
    try {
...
export const getAllActiveFieldTrips = async (req, res) => {
    try {
        ...
        const formattedTrips = activeFieldTrips.map((trip) => {
            ...
            return {
                ...
                daysRemaining: Math.ceil((trip.endDate.getTime() - targetDate.getTime()) /
                    (1000 * 60 * 60 * 24)),
            };
        });
        res.status(200).json({
...
export const updateFieldTrip = async (req, res) => {
    try {
        const { fieldTripKey } = req.params;
